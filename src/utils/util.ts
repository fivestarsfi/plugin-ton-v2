...
export async function waitSeqno(seqno: number, wallet: any) {
...
export async function waitSeqnoContract(seqno: number, contract: any) {
    for (let attempt = 0; attempt < 10; attempt++) {
        await sleep(2000);
...
export function sanitizeTonAddress(
    input: string,
    bounceable?: boolean,
    testOnly?: boolean
): string | null {
    try {
        ...
        const sanitizedAddress = address.toString({
...
export function convertToBigInt(input: string | number): bigint {
    ...
    const cleanedInput =
        typeof input === "string" ? input.replace(/_/g, "") : input;
