...
export async function getListingData(
    ...
    try {
        ...
        throw new Error(
            `Failed to get listing data: ${error instanceof Error ? error.message : String(error)}`
        );
    }
}

function parseFixedPriceData(
...
export async function getFixedPriceData(
    walletProvider: WalletProvider,
    nftAddress: string
): Promise<FixedPriceData & { listingAddress: Address }> {
    try {
        const listingAddress = await getNftOwner(walletProvider, nftAddress);
        const client = walletProvider.getWalletClient();
        const result = await client.runMethod(listingAddress, "get_sale_data");
        ...
        throw new Error(
            `Failed to get fixed price data: ${error instanceof Error ? error.message : String(error)}`
        );
    }
}
...
export async function getAuctionData(
    walletProvider: WalletProvider,
    nftAddress: string
): Promise<AuctionData & { listingAddress: Address }> {
    try {
        const listingAddress = await getNftOwner(walletProvider, nftAddress);
        const client = walletProvider.getWalletClient();
        const result = await client.runMethod(listingAddress, "get_sale_data");
        ...
        throw new Error(
            `Failed to get auction data: ${error instanceof Error ? error.message : String(error)}`
        );
    }
}
...
export async function getNextValidBidAmount(
    walletProvider: WalletProvider,
    nftAddress: string
): Promise<bigint> {
    const listingData = await getListingData(walletProvider, nftAddress);
    ...
    const minIncrement =
        (listingData.lastBid * BigInt(auctionData.minStep)) / BigInt(100);
