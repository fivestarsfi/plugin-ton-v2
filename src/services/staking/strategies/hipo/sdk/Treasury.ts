...
export const sortedDictionaryValue: DictionaryValue<
    Dictionary<bigint, unknown>
> = {
    ...
    parse: function (src: Slice): Dictionary<bigint, unknown> {
        return src
            .loadRef()
            .beginParse()
            .loadDictDirect(Dictionary.Keys.BigUint(256), emptyDictionaryValue);
    },
};
...
export const participationDictionaryValue: DictionaryValue<Participation> = {
    ...
    parse: function (src: Slice): Participation {
        return {
            ...
            sorted: src.loadDict(
                Dictionary.Keys.BigUint(112),
                sortedDictionaryValue
            ),
            requests: src.loadDict(
                Dictionary.Keys.BigUint(256),
                requestDictionaryValue
            ),
            rejected: src.loadDict(
                Dictionary.Keys.BigUint(256),
                requestDictionaryValue
            ),
            accepted: src.loadDict(
                Dictionary.Keys.BigUint(256),
                requestDictionaryValue
            ),
            accrued: src.loadDict(
                Dictionary.Keys.BigUint(256),
                requestDictionaryValue
            ),
            staked: src.loadDict(
...
export class Treasury implements Contract {
    ...
    async getTreasuryState(
        ...
        return {
            ...
            loanCodes: Dictionary.loadDirect(
                Dictionary.Keys.BigUint(32),
            ...
            billCodes: Dictionary.loadDirect(
                Dictionary.Keys.BigUint(32),
                Dictionary.Values.Cell(),
                stack.readCell()
            ),
            oldParents: Dictionary.loadDirect(
                Dictionary.Keys.BigUint(256),
