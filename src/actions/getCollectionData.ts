...
function isGetCollectionDataContent(
    content: Content
): content is GetCollectionDataContent {
 ...
 */
const getCollectionDataTemplate = `Extract the NFT collection information from the conversation.

{{recentMessages}}

Extract the collection address (TON address) for which to fetch NFT collection data.

Respond with the extracted values in this XML format:
<values>
<collectionAddress>TON_ADDRESS_HERE</collectionAddress>
</values>`;
...
class GetCollectionDataAction {
     ...
     */
    async getData(collectionAddress: string): Promise<{
        collectionAddress: string;
        ...
        try {
            ...
            const collectionDataResult = await walletClient.runMethod(
                addr,
                "get_collection_data"
            );
            elizaLogger.log(
                `Collection data result: ${safeStringify(collectionDataResult)}`
            );
            ...
            try {
                ...
                elizaLogger.error("Error reading owner address:");
            ...
            try {
                ...
                const royaltyResult = await walletClient.runMethod(
                    addr,
                    "royalty_params"
                );
                elizaLogger.log(
                    `Royalty result: ${safeStringify(royaltyResult)}`
                );

                const numerator = royaltyResult.stack.readNumber();
                const denominator = royaltyResult.stack.readNumber();
                const destination = royaltyResult.stack
                    .readAddress()
                    .toString();
                ...
                elizaLogger.error("Error fetching royalty parameters:");
            }

            // Get NFT items by index
            const nftItems = [];
            elizaLogger.log(
                `Collection has ${nextItemIndex} NFT items. Fetching addresses...`
            );

            for (let i = 0; i < nextItemIndex; i++) {
                try {
                    const nftAddressResult = await walletClient.runMethod(
                        addr,
                        "get_nft_address_by_index",
                        [{ type: "int", value: BigInt(i) }]
                    );

                    const nftAddress = nftAddressResult.stack
                        .readAddress()
                        .toString();
                    nftItems.push({
                        index: i,
                    ...
                    elizaLogger.error(
                        `Error fetching NFT address for index ${i}:`
                    );
                }
            }
...
const buildGetCollectionData = async (
    ...
    const response = await runtime.useModel(ModelType.SMALL, {
        prompt: getCollectionContext,
    });

    const parsedResponse = parseKeyValueXml(response);

    const buildGetCollectionDataContent: GetCollectionDataContent = {
        collectionAddress: parsedResponse?.collectionAddress || "",
    };
...
export default {
    ...
    handler: async (
        ...
        try {
            ...
            const getCollectionDetails = await buildGetCollectionData(
                runtime,
                message,
            ...
            const getCollectionDataAction = new GetCollectionDataAction(
                walletProvider
            );
            const collectionData = await getCollectionDataAction.getData(
                getCollectionDetails.collectionAddress
            );

            // Format a user-friendly response
            const nftItemsText =
                collectionData.nftItems.length > 0
