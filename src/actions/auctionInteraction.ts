...
// Remove schema as we'll parse XML response directly
...
const auctionInteractionTemplate = `Extract the auction interaction information from the conversation.

{{recentMessages}}

Extract the auction interaction details:
- auctionAddress: The auction contract address
- auctionAction: One of getAuctionData, bid, stop, cancel, list, buy, changePrice, addValue, cancelOffer, getOfferData
- bidAmount: Bid amount in TON (for bid action)
- senderAddress: Sender's TON address (for actions other than getAuctionData)
- nftAddress: NFT address for listing
- fullPrice: Full price in TON
- marketplaceAddress: Marketplace address
- marketplaceFeeAddress: Fee recipient address
- marketplaceFeePercent: Marketplace fee percentage (number)
- royaltyAddress: Royalty recipient address
- royaltyPercent: Royalty percentage (number)
...
<values>
<auctionAddress>AUCTION_ADDRESS</auctionAddress>
<auctionAction>ACTION_TYPE</auctionAction>
<bidAmount>AMOUNT_OR_EMPTY</bidAmount>
<senderAddress>SENDER_ADDRESS_OR_EMPTY</senderAddress>
<nftAddress>NFT_ADDRESS_OR_EMPTY</nftAddress>
<fullPrice>PRICE_OR_EMPTY</fullPrice>
<marketplaceAddress>MARKETPLACE_ADDRESS_OR_EMPTY</marketplaceAddress>
<marketplaceFeeAddress>FEE_ADDRESS_OR_EMPTY</marketplaceFeeAddress>
<marketplaceFeePercent>FEE_PERCENT_OR_EMPTY</marketplaceFeePercent>
<royaltyAddress>ROYALTY_ADDRESS_OR_EMPTY</royaltyAddress>
<royaltyPercent>ROYALTY_PERCENT_OR_EMPTY</royaltyPercent>
<newPrice>NEW_PRICE_OR_EMPTY</newPrice>
<additionalValue>ADDITIONAL_VALUE_OR_EMPTY</additionalValue>
</values>`;
...
const buildAuctionInteractionData = async (
    ...
    return {
        auctionAddress: parsedResponse?.auctionAddress || "",
        auctionAction:
            (parsedResponse?.auctionAction as any) || "getAuctionData",
        bidAmount: parsedResponse?.bidAmount || undefined,
        senderAddress: parsedResponse?.senderAddress || undefined,
        nftAddress: parsedResponse?.nftAddress || undefined,
        fullPrice: parsedResponse?.fullPrice || undefined,
        marketplaceAddress: parsedResponse?.marketplaceAddress || undefined,
        marketplaceFeeAddress:
            parsedResponse?.marketplaceFeeAddress || undefined,
        marketplaceFeePercent: parsedResponse?.marketplaceFeePercent
            ? parseFloat(parsedResponse.marketplaceFeePercent)
            : undefined,
        royaltyAddress: parsedResponse?.royaltyAddress || undefined,
        royaltyPercent: parsedResponse?.royaltyPercent
            ? parseFloat(parsedResponse.royaltyPercent)
            : undefined,
        newPrice: parsedResponse?.newPrice || undefined,
        additionalValue: parsedResponse?.additionalValue || undefined,
    } as AuctionInteractionContent;
};
...
export class AuctionInteractionAction {
    ...
    async getAuctionData(auctionAddress: string): Promise<any> {
        ...
        try {
            ...
            elizaLogger.error("Error parsing sale data:");
    ...
    async list(params: AuctionInteractionContent): Promise<any> {
        ...
        const listMessage = internal({
            to: Address.parse(params.nftAddress!),
...
export default {
    ...
    handler: async (
        ...
        const params = await buildAuctionInteractionData(
            runtime,
            message,
            state
        );
        ...
        try {
            ...
            switch (params.auctionAction) {
                case "getAuctionData":
                    result = await auctionAction.getAuctionData(
                ...
                case "addValue":
                    ...
                    break;
                case "cancelOffer":
                    result = await auctionAction.cancelOffer(
                        params.auctionAddress
                    );
                    break;
                case "getOfferData":
                    result = await auctionAction.getOfferData(
                        params.auctionAddress
                    );
                    break;
                default:
