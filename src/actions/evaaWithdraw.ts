...
const withdrawTemplate = `<system>
Analyze the conversation and extract the following information about the requested withdrawal operation:
- Amount to withdraw
- Asset to withdraw (USDT, USDC, or TON)
...
<message>
Extract the withdrawal details from the conversation above.
            ...
            const withdrawMessage = this.evaa!.createWithdrawMessage({
                ...
                amount:
                    tonAsset.name === "TON"
                        ...
                        : convertToBigInt(Number(params.amount) * 1e6), //0xFFFFFFFFFFFFFFFFn,
...
const withdrawAction: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            const withdrawDetails: WithdrawContent = {
            ...
            const validatedContent = withdrawSchema.parse(withdrawDetails);
            elizaLogger.debug(
                `Withdraw details: ${JSON.stringify(validatedContent)}`
            );

            if (!isWithdrawContent(validatedContent)) {
                throw new Error("Invalid withdrawing parameters");
            }

            const walletProvider = await initWalletProvider(runtime);
            const action = new WithdrawAction(walletProvider);
            const withdrawResult = await action.withdraw(
                validatedContent,
                runtime,
                callback!
            );
            ...
            if (callback) {
                let responseText = `Successfully initiated withdrawing of ${validatedContent.amount} ${withdrawResult.asset}.`;

                // Add interest information if requested
                if (validatedContent.showInterest) {
                    const formattedDailyInterest = Number(
                        withdrawResult.dailyInterest
                    ).toFixed(4);
                    const formattedAnnualRate = (
                        Number(withdrawResult.annualInterestRate) * 100
                    ).toFixed(2); //(withdrawResult.annualInterestRate * 100).toFixed(2);
                ...
                callback({
                    ...
                    metadata: {
                        ...
                        annualInterestRate:
                            withdrawResult.annualInterestRate.toString(),
                        action: "WITHDRAW",
                    },
                });
            }
            ...
            elizaLogger.error("Error in WITHDRAW_TON handler:");
            if (callback) {
                callback({
                    text: `Failed to withdraw TON: ${error instanceof Error ? error.message : String(error)}`,
                    error: true,
                });
            }
            return false;
