        ...
        if (user.data?.type != "active") {
            ...
            for (const userAsset of this.userAssets) {
                ...
                elizaLogger.debug(
                    "Asset Balance",
                    principal
                        ? (Number(principal.toString()) / Number(ONE)).toFixed(
                              2
                          )
                        : "0.00"
                );
                elizaLogger.debug(
                    ...
                    borrowPrincipal
                        ? (
                              Number(borrowPrincipal.toString()) / Number(ONE)
                          ).toFixed(2)
                        : "0.00"
                );
                elizaLogger.debug(
                    ...
                    supplyPrincipal
                        ? (
                              Number(supplyPrincipal.toString()) / Number(ONE)
                          ).toFixed(2)
                        : "0.00"
                );
                elizaLogger.debug(
                ...
                elizaLogger.debug(
                    "Asset Borrow Balance",
                    borrowPrincipal ? borrowPrincipal.toString() : "0"
                );
                elizaLogger.debug(
                    "Asset Supply Balance",
                    supplyPrincipal ? supplyPrincipal.toString() : "0"
                );
                elizaLogger.debug(
                    "-------------------------------------------------"
                );
                elizaLogger.debug(
                    "Asset Balance",
                    principal ? formatCurrency(fromNano(principal), 2) : "0.00"
                );
                elizaLogger.debug(
                    "Asset Borrow Balance",
                    borrowPrincipal
                        ? formatCurrency(fromNano(borrowPrincipal), 2)
                        : "0.00"
                );
                elizaLogger.debug(
                    "Asset Supply Balance",
                    supplyPrincipal
                        ? formatCurrency(fromNano(supplyPrincipal), 2)
                        : "0.00"
                );
                elizaLogger.debug(
                    "Borrow Interest",
                ...
                elizaLogger.debug(
                    ...
                    formatCurrency(
                        fromNano(dailySupplyInterestFP / 10n ** 13n),
                        6
                    )
                );
                elizaLogger.debug(
                    "Annual Rate:",
                    formatCurrency(fromNano(annualRateFP / 100n), 2)
                );
