...
function isMintContent(content: Content): content is MintContent {
    elizaLogger.log("Validating mint content:");
 ...
 */
const mintNFTTemplate = `<system>
Analyze the conversation and extract the following information about the requested NFT minting:
- NFT type: "collection" or "standalone"
- Collection address (for standalone NFTs)
- Owner address
...
<message>
Extract the NFT minting details from the conversation above.
Respond with the following format:

<nftType>collection_or_standalone</nftType>
<collection>collection_address_if_standalone</collection>
<owner>owner_address</owner>
<storage>file_or_prompt</storage>
...
<metadataName>nft_name</metadataName>
<metadataDescription>nft_description</metadataDescription>
<metadataImage>image_url</metadataImage>
</message>`;
...
const buildMintDetails = async (
    ...
    try {
        ...
        const metadata =
            parsedContent && parsedContent.metadataName
                ? {
                      ...
                      description:
                          parsedContent.metadataDescription || undefined,
                      image: parsedContent.metadataImage || "",
                  }
                : undefined;

        const mintContent: MintContent = {
            nftType:
                (parsedContent?.nftType as "collection" | "standalone") ||
                "standalone",
            collection: parsedContent?.collection || undefined,
            owner: parsedContent?.owner || "",
            storage: (parsedContent?.storage as "file" | "prompt") || "prompt",
        ...
        const validatedContent = mintNFTSchema.parse(mintContent);
        return validatedContent as MintContent;
    } catch (error) {
        elizaLogger.error("Error generating mint content:");
        throw new Error(
            `Failed to generate mint content: ${error instanceof Error ? error.message : String(error)}`
        );
    }
};
...
class MintNFTAction {
    ...
    private async uploadContent(
        ...
        try {
            if (params.storage === "file") {
                ...
                imagesIpfsHash = await uploadFolderToIPFS(
                    params.imagesFolderPath
                );
                elizaLogger.log(
                ...
                elizaLogger.log(
                    `Successfully uploaded metadata to IPFS: ${metadataIpfsHash}`
                );
            ...
            elizaLogger.error("Error uploading content to IPFS:");
            throw new Error(
                `Failed to upload content: ${error instanceof Error ? error.message : String(error)}`
            );
        }
    }
    ...
    private async deployStandaloneNFT(params: MintContent): Promise<any> {
        if (!params.collection) {
            throw new Error(
                "Collection address is required for standalone NFTs"
            );
        }

        try {
            elizaLogger.log(
                `Reading metadata files from ${params.metadataFolderPath}`
            );
            ...
            elizaLogger.log(
                `Found ${files.length} NFT metadata files to deploy`
            );
            elizaLogger.log("Topping up wallet balance...");
            let seqno = await topUpBalance(
                this.walletProvider,
                files.length,
                params.collection
            );
            ...
            for (const file of files) {
                elizaLogger.log(
                    `Starting deployment of NFT ${index + 1}/${files.length}`
                );
                const mintParams = {
                    queryId: 0,
                ...
                elizaLogger.log(
                    `Successfully deployed NFT ${index + 1}/${files.length} with address: ${nftAddress}`
                );
            ...
            elizaLogger.error("Error deploying standalone NFT:");
            throw new Error(
                `Failed to deploy standalone NFT: ${error instanceof Error ? error.message : String(error)}`
            );
        }
    }
     ...
     */
    private async deployCollection(
        params: MintContent,
        ...
        try {
            ...
            elizaLogger.log(
                `Collection deployment transaction sent, waiting for confirmation...`
            );
            ...
            await waitSeqnoContract(seqno, contract);
            elizaLogger.log(
                `Collection successfully deployed: ${collection.address}`
            );

            return collection.address.toString();
        } catch (error) {
            elizaLogger.error("Error deploying NFT collection:");
            throw new Error(
                `Failed to deploy NFT collection: ${error instanceof Error ? error.message : String(error)}`
            );
        }
    }
    ...
    async mint(params: MintContent): Promise<string> {
        try {
            elizaLogger.log(
                `Starting NFT minting process for type: ${params.nftType}`
            );
            elizaLogger.log(`Using storage type: ${params.storage}`);

            const { metadataIpfsHash } = await this.uploadContent(params);
            elizaLogger.log(
                `Content uploaded to IPFS with hash: ${metadataIpfsHash}`
            );

            if (params.nftType === "standalone") {
                elizaLogger.log(
                    `Deploying standalone NFT to collection: ${params.collection}`
                );
            ...
            elizaLogger.error("Error in mint method:");
            throw new Error(
                `Mint operation failed: ${error instanceof Error ? error.message : String(error)}`
            );
        }
    }
}
...
export default {
    ...
    handler: async (
        ...
        try {
            ...
            let mintParams = await buildMintDetails(
                runtime,
                message,
            ...
            elizaLogger.log("NFT minted successfully:");
            ...
            elizaLogger.error("Error minting NFT:");
            if (callback) {
                callback({
