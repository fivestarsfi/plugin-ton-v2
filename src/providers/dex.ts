...
export class DexProvider implements DexProvider {
    ...
    constructor(
        ...
        elizaLogger.log(
            `DexProvider: Initializing with wallet provider on ${network} network`
        );

        try {
            elizaLogger.log(
                `DexProvider: Creating TorchFinance instance on ${network} network`
            );
            ...
            elizaLogger.log(
                "DexProvider: All DEX instances created successfully"
            );
        } catch (error) {
            elizaLogger.error(
                "DexProvider: Error initializing DEX providers:",
                error
            );
            throw error;
        }
    ...
    async createPool(params: {
        ...
        try {
            ...
            elizaLogger.error(
                `DexProvider: Error creating pool on ${dex}:`,
                error
            );
            throw error;
        }
    ...
    isOperationSupported(
        ...
        elizaLogger.log(
            `DexProvider: Operation ${operation} on ${dex} supported: ${supported}`
        );
        return supported;
    }
    ...
    async depositLiquidity(params: {
        ...
        try {
            ...
            elizaLogger.log(
                `DexProvider: Detailed deposit parameters for ${dex}:`,
                {
                    dex: dex,
                    isTon: isTon,
                    tonAmount: tonAmount,
            ...
            elizaLogger.log(
                `DexProvider: Calling deposit method on ${dex.toUpperCase()} DEX instance`
            );
            ...
            elizaLogger.error(
                `DexProvider: Error depositing on ${dex}:`,
                error
            );
            ...
            if (error instanceof Error && error.message) {
                elizaLogger.error(
                    `DexProvider: Error message: ${error.message}`
                );

                if (error.message.includes("exit_code:")) {
                    const exitCodeMatch =
                        error.message.match(/exit_code: (-?\d+)/);
                    const exitCode = exitCodeMatch
                        ? exitCodeMatch[1]
                        : "unknown";
                    elizaLogger.error(
                        `DexProvider: Contract execution error with exit_code: ${exitCode}`
                    );

                    if (exitCode === "-13") {
                        elizaLogger.error(
                            "DexProvider: Exit code -13 typically indicates insufficient balance, non-existent pool, or incorrect parameters"
                        );
                        elizaLogger.error(
                            "DexProvider: Checking if tokens exist in the pool..."
                        );
                        ...
                        if (dexInstance) {
                            elizaLogger.log(
                                `DexProvider: DEX instance type: ${dexInstance.constructor.name}`
                            );
                        }
                    }
                }
            ...
            if (error instanceof Error && error.stack) {
                elizaLogger.error(
                    `DexProvider: Error stack trace: ${error.stack}`
                );
            }
    ...
    async withdrawLiquidity(params: {
        ...
        try {
            ...
            elizaLogger.error(
                `DexProvider: Error withdrawing from ${dex}:`,
                error
            );
            throw error;
        }
    ...
    async claimFees(params: {
        ...
        try {
            const result =
                await this.megaDex[dex.toUpperCase()].claimFee(params);
            elizaLogger.log(`DexProvider: Fee claim result: ${result}`);
            return result;
        } catch (error) {
            elizaLogger.error(
                `DexProvider: Error claiming fees from ${dex}:`,
                error
            );
            throw error;
        }
