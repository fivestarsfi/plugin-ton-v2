...
elizaLogger.log(
    "TorchFinance: Configurations initialized for mainnet and testnet"
);
...
export class TorchFinance implements DEX {
    ...
    async send(args: SenderArguments | SenderArguments[]): Promise<string> {
        ...
        try {
            ...
            const contract = await walletClient.open(
            ...
            const transfer = contract.createTransfer({
                ...
                messages: args.map((arg) => {
                    elizaLogger.log(
                        `TorchFinance: Creating message to ${arg.to.toString()} with value ${arg.value}`
                    );
                    return internal({
                        to: arg.to,
                        value: arg.value,
            ...
            elizaLogger.log(
                "TorchFinance: Transfer message created, sending to network"
            );
            await contract.send(transfer);

            elizaLogger.log(
                `TorchFinance: Transaction sent, waiting for confirmation (seqno: ${seqno})...`
            );
            ...
            elizaLogger.error(
                "TorchFinance: Error sending transaction:"
            );
            throw error;
        }
    }
    ...
    constructor(
        ...
        elizaLogger.log(
            `TorchFinance: Initializing with wallet provider on ${network} network`
        );
        ...
        if (network === "mainnet" && !networkConfig.factoryAddress) {
            elizaLogger.warn(
                "TorchFinance: Mainnet factory address not provided, some operations may not work"
            );
        }

        const tonClient =
            this.walletProvider.getWalletClient() as unknown as TonClient4;
        ...
        elizaLogger.log(
            `TorchFinance: Creating TorchSDK instance with ${network} config:`
        );
    ...
    async deposit(
        ...
        try {
            ...
            for (const jettonDeposit of jettonDeposits) {
                ...
                elizaLogger.log(
                    `TorchFinance: Validated supported token: ${jettonAddress}`
                );
            }
            ...
            if (tonAmount) {
                elizaLogger.log(
                    `TorchFinance: Adding TON to deposit, amount: ${tonAmount}`
                );
            ...
            elizaLogger.log(
                "TorchFinance: Deposit parameters prepared:"
            );
            ...
            try {
                const { result, getDepositPayload } =
                    await this.sdk.simulateDeposit(depositParams);

                elizaLogger.log(
                    `LP Tokens Out: ${result.lpTokenOut.toString()}`
                );
                elizaLogger.log(
                    `LP Total Supply After: ${result.lpTotalSupplyAfter.toString()}`
                );
                elizaLogger.log(`Min LP Tokens Out: ${result.minLpTokenOut?.toString() || '(You did not specify slippage tolerance)'}`); // prettier-ignore

                elizaLogger.log(
                    `Get ${result.lpTokenOut.toString()} LP from ${TRITON_POOL_V2}`
                );
                // Skip simulation as it's causing issues
                elizaLogger.log(
                    `TorchFinance: Getting deposit payload directly for sender: ${sender.toString()}`
                );
                ...
                elizaLogger.error(
                    "TorchFinance: Failed to generate deposit payload:"
                );
                throw new Error(
                    `Failed to generate deposit payload: ${error instanceof Error ? error.message : String(error)}`
                );
            }
            ...
            elizaLogger.log(
                "TorchFinance: Deposit payload generated, sending transaction"
            );
            return await this.send(senderArgs);
        } catch (error) {
            elizaLogger.error("TorchFinance: Error in deposit method:");
            throw error;
        }
    }
    ...
    async withdraw(
        ...
        try {
            ...
            if (isTon && !jettonWithdrawals) {
                ...
                elizaLogger.log(
                    `TorchFinance: Withdrawing jetton: ${jettonAddress}`
                );
                ...
                elizaLogger.error(
                    "TorchFinance: Invalid withdrawal configuration"
                );
                throw new Error("Invalid withdrawal configuration");
            }

            elizaLogger.log(
                "TorchFinance: Withdraw parameters prepared:"
            );
            ...
            try {
                // Skip simulation as it's causing issues
                elizaLogger.log(
                    `TorchFinance: Getting withdraw payload directly for sender: ${sender.toString()}`
                );
                senderArgs = await this.sdk.getWithdrawPayload(
                    sender,
                    withdrawParams,
                    {
                        blockNumber: blockNumber,
                ...
                elizaLogger.error(
                    "TorchFinance: Failed to generate withdraw payload:"
                );
                throw new Error(
                    `Failed to generate withdraw payload: ${error instanceof Error ? error.message : String(error)}`
                );
            }
            ...
            elizaLogger.log(
                "TorchFinance: Withdraw payload generated, sending transaction"
            );
            return await this.send(senderArgs);
        } catch (error) {
            elizaLogger.error("TorchFinance: Error in withdraw method:");
            throw error;
        }
    }
