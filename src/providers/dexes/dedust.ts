...
import { JettonMaster, Sender, toNano, TonClient } from "@ton/ton";
...
export class Dedust implements DEX {
    ...
    private async initSender() {
        ...
        try {
            ...
            elizaLogger.log(
                `Dedust: Sender initialized with address: ${this.walletProvider.wallet.address.toString()}`
            );
        } catch (error) {
            elizaLogger.error("Dedust: Error initializing sender:");
            throw error;
        }
    }
    ...
    async createPool(jettons: JettonMaster[]) {
        elizaLogger.log(
            "Dedust: Creating pool with jettons:"
        );
        ...
        try {
            ...
            elizaLogger.log(
                `Dedust: Pool type: ${isTon ? "TON + Jetton" : "Jetton + Jetton"}`
            );
            ...
            elizaLogger.log(
                `Dedust: Assets: ${assets[0].toString()}, ${assets[1].toString()}`
            );
            ...
            const hasVaults = await Promise.all(
                jettons.map(async (jetton, index) => {
                    const hasVault = await !!this.factory.getJettonVault(
                        jetton.address
                    );
                    elizaLogger.log(
                        `Dedust: Vault for jetton ${index + 1} exists: ${hasVault}`
                    );
                    return hasVault;
                })
            );
            ...
            for (const jetton of jettons) {
                ...
                if (hasVaults[index]) {
                    elizaLogger.log(
                        `Dedust: Vault for jetton ${index + 1} already exists, skipping creation`
                    );
                    continue;
                }

                elizaLogger.log(
                    `Dedust: Creating vault for jetton ${index + 1}: ${jetton.address.toString()}`
                );
                ...
                elizaLogger.log(
                    `Dedust: Vault creation initiated for jetton ${index + 1}, txHash: ${txHash}`
                );
            }
            ...
            if (poolReadiness === ReadinessStatus.NOT_DEPLOYED) {
                ...
                try {
                    const txHash = await this.factory.sendCreateVolatilePool(
                        this.sender,
                        {
                            assets,
                        }
                    ...
                    elizaLogger.log(
                        `Dedust: Pool creation initiated, txHash: ${txHash}`
                    );
                    return txHash;
                } catch (error) {
                    elizaLogger.error("Dedust: Error creating pool:");
                    return false;
                }
            }
            ...
            elizaLogger.error("Dedust: Error in createPool method:");
            throw error;
        }
    }
    ...
    async deposit(
        ...
        try {
            ...
            elizaLogger.log(
                `Dedust: Deposit type: ${isTon ? "TON + Jetton" : "Jetton + Jetton"}`
            );
            ...
            const assets: [Asset, Asset] = [
                isTon
                    ? Asset.native()
                    : Asset.jetton(jettonDeposits[0].jetton.address),
            ...
            elizaLogger.log(
                `Dedust: Target balances: ${targetBalances[0].toString()}, ${targetBalances[1].toString()}`
            );
            ...
            if (isTon) {
                // Deposit ton to pool
                elizaLogger.log(
                    `Dedust: Depositing TON (${tonAmount}) to pool`
                );
                const TON = Asset.native();
                const tonVault = this.tonClient.open(
                    await this.factory.getNativeVault()
                );

                elizaLogger.log(
                    `Dedust: TON vault address: ${tonVault.address.toString()}`
                );
                ...
                elizaLogger.log(
                    `Dedust: TON deposit initiated, txHash: ${txHash}`
                );
            }

            // Deposit either a single or two jettons to a pool
            elizaLogger.log(
                `Dedust: Depositing ${jettonDeposits.length} jettons to pool`
            );
            ...
            for (const jettonDeposit of jettonDeposits) {
                ...
                elizaLogger.log(
                    `Dedust: Processing jetton: ${jettonDeposit.jetton.address.toString()}, amount: ${jettonDeposit.amount}`
                );
                ...
                elizaLogger.log(
                    `Dedust: Jetton vault address: ${assetVault.address.toString()}`
                );

                const assetWallet = this.tonClient.open(
                    await assetContract.getWallet(
                        this.sender?.address ||
                            this.walletProvider.wallet.address
                    )
                );
                elizaLogger.log(
                    `Dedust: Jetton wallet address: ${assetWallet.address.toString()}`
                );

                elizaLogger.log(
                    `Dedust: Transferring ${jettonDeposit.amount} jettons to vault`
                );
                ...
                txHashes.push(txHash);
                elizaLogger.log(
                    `Dedust: Jetton transfer initiated, txHash: ${txHash}`
                );
            }

            elizaLogger.log(
                `Dedust: All deposit operations initiated with txHashes: ${txHashes.join(", ")}`
            );
            return txHashes[0]; // Return the first hash for compatibility
        } catch (error) {
            elizaLogger.error("Dedust: Error in deposit method:");
            throw error;
        }
    }
    ...
    async withdraw(
        ...
        try {
            ...
            const lpWallet = this.tonClient.open(
                await pool.getWallet(this.sender.address)
            );
            elizaLogger.log(
                `Dedust: LP wallet address: ${lpWallet.address.toString()}`
            );

            const lpBalance = await lpWallet.getBalance();
            elizaLogger.log(
                `Dedust: LP wallet balance: ${lpBalance.toString()}`
            );

            const burnAmount = toNano(amount);
            elizaLogger.log(
                `Dedust: Burning ${amount} LP tokens (${burnAmount.toString()})`
            );

            // Use sendBurn directly with the sender
            const txHash = await lpWallet.sendBurn(this.sender, burnAmount, {
                amount: lpBalance,
            });

            elizaLogger.log(
                `Dedust: LP token burn initiated, txHash: ${txHash}`
            );
            return txHash;
        } catch (error) {
            elizaLogger.error("Dedust: Error in withdraw method:");
            throw error;
        }
    }
    ...
    async getPool(jettons: JettonMaster[]) {
        elizaLogger.log(
            "Dedust: Getting pool for jettons:"
        );
        ...
        try {
            ...
            elizaLogger.error("Dedust: Error getting pool:");
            return undefined;
        }
    }
}
